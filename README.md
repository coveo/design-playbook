...
- **Plays as content** — every play is one MDX file in `packages/design-playbook/plays/` with a frontmatter contract (section, confidence level, Miro template, agent recipe…). Files are auto-discovered; there is no registry to edit.
...
Create `packages/design-playbook/plays/<slug>.mdx` following the contract in [AGENTS.md](./AGENTS.md) — or open the repo in Claude Code and ask for the `add-play` skill. Play bodies follow the playbook grammar: *When? / Why? / What do you need? / Step by step / Common mistakes / What next?*
