...
- **Workshop skills** — `skills/` contains skills that *run* plays, not just edit them: `run-design-smash`, `run-shaping-workshop`, `run-design-dash`, and `run-journey-mapping` each interview you and scaffold the session's Miro board from their play's steps via the Miro MCP. `add-play` scaffolds new plays. (`.claude/skills` symlinks here so Claude Code auto-discovers them.)
